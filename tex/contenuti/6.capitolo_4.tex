...
In questo capitolo si configurerà il \textit{Router 4g} connettendolo alla VPN.

Continuando a seguire un approccio incrementale, verranno ignorati per ora gli \textit{host-domotici} e verrà aggiunto alla topologia solo il \textit{Router 4G}. Quindi, la configurazione di partenza sarà descritta in fig.~\ref{fig:conf-init-router}.
...
Possiamo quindi spostarci nel \textit{Router} e connetterlo alla VPN:
...
Dopodiché va abilitata usando \textit{LuCI}:
...
In questo momento i client della VPN, \code{client1} e \code{router}, possono comunicare tra loro. Ma lo fanno passando per la network stack del \textit{Server}, questo può essere verificato con le utility \href{https://en.wikipedia.org/wiki/Ping_(networking_utility)}{\textit{ping}} e \href{https://en.wikipedia.org/wiki/Tcpdump}{\textit{tcpdump}}. 
...
Dopo aver riavviato il servizio di openvpn con \code{systemctl restart}, è possibile rieseguire gli stessi test fatti sopra:
...
Nel caso in esame è necessario impostare un IP statico per il \textit{Router}, quindi si deve creare nella cartella \code{ccd} un file chiamato \code{router}. 
