...
Continuando a seguire un approccio incrementale, ignoriamo per ora gli \it{host-domotici} e aggiungiamo alla topologia solo il \it{Router 4G}. Quindi la configurazione di partenza è descritta in fig.\ref{fig:diag-router}.
...
L'obbiettivo di questo capitolo è di configurare il \it{Router 4G} e il \it{Server} in modo da poter instaurare un canale di comunicazione tra \it{Router} e \it{Client}.


\section{Creazione della configurazione Openvpn}

% TODO da riscrivere
Si devono seguire gli step descritti in sezione \ref{sec:client_keys}, quindi creare la certificate request e firmarla nel \textit{server CA}. Per poi usare lo script creato in sezione \ref{sec:script_client} per costruire il file di configurazione:

\begin{bashcode}{Server}{}
$ ./make_config.sh router
\end{bashcode}

Dopodiché si deve spostare il file \code{router.ovpn} dal \textit{Server} al \textit{Router 4g}, supponiamo di averlo copiato nella cartella \code{/configs}. 
...
In questo momento i client della VPN, \textit{client1} e \textit{router}, possono comunicare tra loro, ma lo fanno passando per la network stack del \textit{server}. Infatti:
