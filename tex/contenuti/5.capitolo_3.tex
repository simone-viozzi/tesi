...
\subsection{Creazione della \textit{Certificate Authority} CA \workinprogress} % step 1, 2
...
La generazione dei certificati per i client consiste in una procedura molto simile alla creazione del certificato del \it{Server}, sezione \ref{sec:pki_openvpn} e \ref{subsec:sign_openvpn}.
...
il certificato e chiave relativi al \it{Client} per cui si sta creando la configurazione, usando il \it{Common Name} passato come argomento; 
...
Ciò creerà il file di configurazione per \it{client1} scrivendolo in \\\code{client-configs/files/client1.ovpn}, successivamente sarà necessario copiare questo file nella macchina del \it{client}.
...
Per farlo ci spostiamo su una macchina \it{Client}, con sistema operativo Linux ad esempio. Usando la configurazione creata in code:\ref{code:config-client1} possiamo connettere il \textit{Client} alla VPN:
