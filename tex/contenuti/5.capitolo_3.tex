...
\todo[non supponiamo, da cambiare]
...
Per concludere la procedura (step 7 fig.\ref{fig:diag-firma_certificato_ca}), si devono copiare i file \code{ca.crt} e \code{server.crt} dal \textit{Server CA} al \textit{Server OpnenVPN}, spostandoli nella cartella del \it{Server OpenVPN}: \\\code{/etc/openvpn/server}.
...
Con questo comando verranno creati 2 file: \code{pki/private/client1.key} costituisce la chiave privata del certificato e va copiata nella directory creata in code:~\ref{code:clients-config-folder}; 
...
\todo[va bene corsivo per vpn e nat ?]
...
\section{Test della configurazione \ok \todo[si può espandere in qualche modo?]}
