...
Questo script permette di aggiungere in modo \it{inline} i file necessari all'autenticazione di un client alla VPN, % TODO da riscrivere fa pena
ciò è molto comodo dato che sarà sufficiente mandare ai client un solo file invece di 5 file separati.

Quindi lo script userà: il certificato della CA, \code{ca.crt}; 
il certificato e chiave relativi al client per cui si sta creando la configurazione, il nome della configurazione deve essere lo stesso del \it{Common Name} del certificato; 
la \textit{preshared key}, \code{ta.key}.


Lo scopo di questo script è di aggiungere in modo \it{inline} al file \code{base.conf}
il certificato della CA, \code{ca.crt}, 
il certificato e chiave relativi al client per cui si sta creando la configurazione, passato come argomento allo script, 
