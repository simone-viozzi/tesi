...
Sapendo che l'IP pubblico del \it{Server} è \code{51.178.141.119}, si avrà una configurazione iniziale come in figura \ref{fig:diag-simple_ips}.
...
Il \textit{Server CA} verrà usato in fase di configurazione del \it{Server} e di creazione dei certificati per i client, dopodiché non sarà più necessario.
...
\newpage
Quindi si deve spostare/collegare questo percorso in una cartella di un utente non privilegiato, in questo caso \code{ubuntu}. Per semplicità il collegamento verrà creato nella \it{home}:
...
A questo punto si possono creare i certificati per i client. Ad esempio si può creare un certificato per il \it{Client}, è importante che il \textit{Common Name} dato in fase di creazione sia unico ed è consigliabile usare un nome che permetta di collegare il certificato con il cliente che ha richiesto il servizio. Ciò permette di conoscere quale certificato revocare \cite{revoke-cert} in caso sia necessario.
...
OpenVPN per funzionare necessita che il server abbia l'\textit{ip forwarding} abilitato. Ciò è necessario per permettere al \it{server OpnenVPN} di inoltrare il traffico proveniente dalla VPN nella sua network stack, e viceversa.
...
In caso il servizio fallisse ad avviarsi si avrà un breve \it{log} con \code{systemctl status}, oppure si possono leggere i \it{log} completi usando \code{journalctl -u openvpn-server@server.service}.


\section{Test della configurazione \ok}
\label{sec:test_config_server}

Ora che la configurazione per questo capitolo è conclusa è necessario testare che tutto funzioni correttamente.
