...
In questo capitolo verrà installato e configurato \textit{OpenVPN server} sulla VPS di OVHCloud.

Per facilitare la configurazione e il testing, si suppone di partire da una topologia che contiene solo il server OpenVPN e un generico client. Come da specifiche, il Server deve avere a disposizione un'IP pubblico (sezione \ref{subsec:vps-ovhcloud}); e il client deve essere il più generico possibile, viene quindi supposto al di sotto di un NAT (sezione \ref{subsec:macchina-cliente}).

Sapendo che l'IP pubblico del \textit{Server} è \code{51.178.141.119}, si avrà una configurazione iniziale come in figura \ref{fig:diag-simple_ips}.
...
\begin{figure}[H]
    ...
    \begin{subfigure}{0.49\textwidth}
        ...
        \caption{Topologia virtuale da raggiungere.}
...
Il \textit{Server CA} verrà usato in fase di configurazione del \textit{Server OpenVPN} e di creazione dei certificati per i client, dopodiché non sarà più necessario.

\begin{figure}[H]
    \centering
    \includegraphics[width=1\linewidth]{immagini/diag-firma_certificato_ca}
    \caption{Diagramma per schematizzare la procedura di creazione della CA e della PKI del server OpenVPN.\todo[va bene senza mettere nessuna spiegazione per lo schema??]}
...
\newpage
Quindi si deve spostare/collegare questo percorso in una cartella di un utente non privilegiato, in questo caso \code{ubuntu}. Per semplicità il collegamento verrà creato nella \textit{home}:
...
A questo punto è possibile usare lo script \code{easyrsa} presente nella cartella per creare la PKI.
...
Si deve quindi usare l'albero di cartelle creato in code:~\ref{code:init-easyrsa}:
...
\newpage
In questo caso verrà creato un nuovo file:  \todo[è ok lasciare queste?]
...
set_var EASYRSA_REQ_EMAIL    "****@esse-ti.it"
...
Le variabili nel primo blocco determinano i dati che poi verranno registrati nei certificati. Le ultime 2 sono opzioni di sicurezza. In particolare si configura il tipo di algoritmo di cifratura per usare la crittografia a chiave ellittica (\href{https://en.wikipedia.org/wiki/Elliptic-curve_cryptography}{Elliptic-Curve Cryptography}); L'ultima opzione setta l'algoritmo di hashing da usare nella firma dei certificati.

A questo punto si deve lanciare il comando \code{build-ca} per costruire la CA (passaggio 2 fig.\ref{fig:diag-firma_certificato_ca}):
...
Quindi per prima cosa si devono ripetere i passaggi fatti in sezione \ref{subsec:pki_structure}, questa volta però la cartella è stata chiamata \code{openvpn-pki} in modo da distinguerle:
...
Si deve quindi creare un file \code{vars}, che questa volta conterrà solo le informazioni essenziali, e poi dare il comando \code{init-pki} (step 3 fig.\ref{fig:diag-firma_certificato_ca}):
...
Per firmare la \textit{Certificate Signing Request} si deve copiare il file \code{.req} nel \textit{Server CA}. Supponendo di averlo copiato nella cartella \code{/tmp}, lo si deve importare nella CA e firmarlo, 

ciò corrisponde ai passaggi 5 e 6 fig.\ref{fig:diag-firma_certificato_ca}:
...
Per aumentare ulteriormente la sicurezza del nostro \textit{Server OpenVPN} si può creare un'ulteriore chiave, che permette di offuscare il certificato in fase di validazione. In questo caso la chiave è di tipo \textit{preshared} ed è comune per tutti gli utenti, serve principalmente per aggiungere un ulteriore livello di sicurezza.
...
Per raccogliere tutte le chiavi degli utenti si creerà una cartella nella home, successivamente questa verrà usata per automatizzare la creazione dei file di configurazione OpenVPN che verranno usati dai client per connettersi alla VPN.
...
A questo punto si possono creare i certificati per i client. Ad esempio si può creare un certificato per il \textit{Client}, è importante che il \textit{Common Name} dato in fase di creazione sia unico ed è consigliabile usare un nome che permetta di collegare il certificato con il cliente che ha richiesto il servizio. Ciò permette di conoscere quale certificato revocare \cite{revoke-cert} in caso sia necessario.
...
Supponiamo di aver copiato il file \code{client1.req} nella cartella \code{tmp} del \textit{Server CA}, è quindi possibile importarla e firmarla (passaggi 2 e 3 fig.\ref{fig:diag-firma_certificato_client}):
...
Per facilitare la creazione dei file di configurazione OpneVPN dei client, \code{Common-Name.ovpn}, andremo a creare un apposito script bash che userà la cartella creata in code:~\ref{code:clients-config-folder}. 

Per concludere la preparazione si devono copiare i file \code{ta.key} (creato nella sezione \ref{sec:tls-crypt}) e \code{ca.crt} (creato nella sezione \ref{subsec:sign_openvpn}), correggendo inoltre l'\textit{owner} in modo che l'utente non privilegiato vi possa accedere:
...
\newpage
Ora si deve scaricare e personalizzare la configurazione di esempio per i client:
...
Ora si può creare lo script bash \code{make_config.sh}:
...
A questo punto si deve modificare il file di esempio personalizzandolo per le nostre necessità:
...
OpenVPN per funzionare necessita che il server abbia l'\textit{ip forwarding} abilitato. Ciò è necessario per permettere al \textit{server OpnenVPN} di inoltrare il traffico proveniente dalla VPN nella sua network stack, e viceversa.
...
In questo caso il nome dell'interfaccia di rete è \textit{ens3}. È quindi possibile procedere con la configurazione del NAT:
...
Se la configurazione è stata effettuata correttamente di avrà un output simile al precedente, in caso contrario si dovranno valutare gli errori dati da \code{ufw}.

Maggiori informazioni sul funzionamento di \code{ufw} possono essere trovare nella sua \href{http://manpages.ubuntu.com/manpages/bionic/man8/ufw-framework.8.html}{documentazione}.

\newpage
\section{Avvio del Server OpenVPN \ok}
\label{sec:start_server}

Ora che la configurazione del Server è in una situazione stabile è possibile avviarlo:
...
In caso il servizio fallisse ad avviarsi si avrà un breve \textit{log} con \code{systemctl status}, oppure si possono leggere i \textit{log} completi usando \code{journalctl -u openvpn-server@server.service}.


\section{Test della configurazione \ok}
\label{sec:test_config_server}

Ora che la configurazione per questo capitolo è conclusa è necessario verificare che tutto funzioni correttamente.

Per farlo ci spostiamo su una macchina \textit{Client}, con sistema operativo Linux ad esempio. Usando la configurazione creata in code:\ref{code:config-client1} è possibile connettere il \textit{Client} alla VPN:
...
Con questi test è possibile concludere che la topologia di obbiettivo per questo capitolo, descritta in fig.\ref{fig:diag-simple_ips_vpn}, sia stata realizzata correttamente.
