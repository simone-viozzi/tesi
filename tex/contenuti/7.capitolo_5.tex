...
Supponiamo per esempio che il \textit{Client} voglia comunicare con un \textit{host-domotico}, in questo caso la \textit{Raspberry Pi} che ha IP \code{192.168.130.2}, e che il dato da trasmettere sia un ICMP \textit{echo request}, facente parte di un \code{ping}.
...
\end{bashcode}
    ...
    \item[3.] Il \textit{Server OpenVPN} verifica che il pacchetto è destinato nella subnet \code{192.168.130.0/24}, data la configurazione fatta in sezione \ref{sec:hosts-openvpn-server}, il \textit{Server} provvederà a inoltrarlo al \textit{Router} (\code{10.8.0.254}).
    ...
    Il pacchetto cifrato viene quindi inoltrato usando come IP di sorgente \code{51.178.141.119} e destinazione l'IP pubblico del NAT.
    \newpage
...
\newpage
Possiamo inoltre verificare il percorso del pacchetto interno (cifrato) usando la utility \code{tracepath}, che permette di vedere l'indirizzo IP di tutti i router che sono presenti tra chi esegue il comando e la destinazione:
...
Come possiamo vedere l'unico router presente nella topologia virtuale è il \it{Router 4G} (\code{10.8.0.254}).
