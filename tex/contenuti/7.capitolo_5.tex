...
L'ultima parte della configurazione consiste nel rendere disponibile nella VPN la rete locale del \it{router 4g}, consentendo quindi lo scambio di dati tra gli host domotici e i client della vpn

Per simulare gli host domotici è stata usata una \it{raspberry pi}.
...
% rifare lo screen in modo che prenda solo la parte al centro
\begin{figure}[H]
    \centering
...
Ciò va fatto da LuCI, nella sezione \it{interfaces}. Si deve quindi aggiungere una nuova interfaccia con le seguenti opzioni:
\begin{itemize}
    \item Name: tun0
    \item Proto: static
    \item Device: tun0
...
Per instaurare una comunicazione bidirezionale tra \it{host-domotico} e \it{client} è necessario che:
...
% TODO da scrivere di piu' per i test, sono importanti, forse mettere qualcosa sul troubble shooting... mhhh


Per testare la connessione tra \it{client} e \it{host-domotico} usiamo sia \code{ping} che \code{tracepath}, possiamo inoltre usare \code{tcpdump} sul \it{Server} e \it{Router} per vedere il percorso dei pacchetti.

Vediamo quindi un ping dal client verso l'\it{host-domotico}, con server e router in ascolto sull'interfaccia tun0:
