...
\end{bashcode}
    ...
    \item[3.] Il \textit{Server OpenVPN} vede che il pacchetto è destinato nella subnet \code{192.168.130.0/24}, data la configurazione fatta in sezione \ref{sec:hosts-openvpn-server}, il \textit{Server} sa che deve inoltrarlo al \textit{Router} (\code{10.8.0.254}).
...
Possiamo inolte verificare il percorso del pacchetto interno (cifrato) usando la utility \code{tracepath}, che permette di vedere tutti i router che sono presenti tra chi eseque il \it{tracepath} e la destinazione:
