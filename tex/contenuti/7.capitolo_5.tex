...
Per attivare la zona firewall aggiunta nella sezione precedente, si deve aggiungere l'interfaccia relativa alla VPN (\textit{tun0}) alla zona \textit{vpn}.
...
A questo punto i pacchetti provenienti dalla \textit{lan} del \textit{Router} vengono inoltrati correttamente nella VPN, ma gli Host nella VPN non hanno una rotta verso la rete interna del router, perciò la comunicazione risulta ancora mono direzionale. È possibile verificarlo con un semplice test i cui:
...
\newpage
Nell'analisi andremo a seguire il percorso del pacchetto nei punti di interesse rappresentati in fig.~\ref{fig:diag-test-real}, esaminando in modo dettagliato cosa succede ad ogni step:
...
$ tcpdump -i tun0
\end{bashcode}
    Possiamo però vedere il pacchetto transitare sull'interfaccia esterna del \textit{Server} (\textit{ens3}). In questo caso però vedremo il pacchetto esterno, possiamo riconoscerlo e distinguerlo dal normale traffico internet selezionando la porta di OpenVPN:
\begin{bashcode}{Server}{}
...
\end{bashcode}
    ...
    Il pacchetto viene cifrato con il certificato del \textit{Router} e inoltrato usando come IP di sorgente \code{51.178.141.119} e destinazione l'IP pubblico del NAT.
    \newpage
    Possiamo vedere il pacchetto in uscita dal \textit{server} mettendoci in ascolto sull'interfaccia \textit{ens3}:
\begin{bashcode}{Server}{}
$ sudo tcpdump -i ens3 -n port 1194
...
Come era previsto l'unico router presente nella topologia virtuale è il \textit{Router 4G} (\code{10.8.0.254}).
