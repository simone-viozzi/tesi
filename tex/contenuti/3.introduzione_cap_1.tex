...
Possiamo vedere l'incapsulamento in azione catturando un pacchetto con il programma \it{Wireshark}, ad esempio in questo caso si tratta di un pacchetto proveniente da una pagina web https:
...
\todo[da sistemare, non so se è l'approccio giusto, penso che ci sia da cercare ancora sull'incapsulamento]
...
\begin{itemize}
    ...
    \item \textbf{Transmission Control Protocol}\cite{RFC_0793}: incapsula il layer applicazione in un layer di trasporto usando il protocollo TCP. 
    % devo di cazzo ci sta dentro?
    
    Aggiunge un minimo di 20 byte di header. \todo
...
Il NAT \cite{RFC_1631} permette di effettuare una mappatura da uno spazio di indirizzi a un altro, modificando così le informazioni di routing nel pacchetto IP. La mappatura può essere sia uno-a-uno, nel caso di \it{Static NAT} e \it{Dynamic NAT}; sia molti-a-molti, nel caso di \it{Network Address and Port Translation} (NAPT), in cui oltre all'IP viene modificata anche la porta del protocollo di trasporto. Ciò consente di mappare un grande numero di indirizzi privati in relativamente pochi indirizzi pubblici. 
...
% TODO forse posso tagliarla un pò non è poi così necessaria
...
I cg-nat sono una tipologia di NAT che lavorano su larga scala e in generale sono implementati all'interno della rete dell'ips.
