...
% TODO non so se è l'approccio giusto, penso che ci sia da cercare ancora sull'incapsulamento
...
Il NAT \cite{RFC_1631} permette di effettuare una mappatura da uno spazio di indirizzi a un altro, modificando così le informazioni di routing nel pacchetto IP. La mappatura può essere sia uno-a-uno, \it{Static NAT} e \it{Dynamic NAT}, sia molti-a-molti, \it{Network Address and Port Translation} (NAPT). Nel caso del NAPT il pool di indirizzi privati è molto più grande rispetto al pool di indirizzi pubblici, ciò consente di riutilizzare gli stessi ip pubblici per multipli utenti.
